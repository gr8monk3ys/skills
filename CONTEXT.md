...
- **Ships vs runs** — the repo root (`commands/`, `agents/`, `skills/`, `hooks/`) is **what ships to users**; `.claude/` is **how this repo itself runs** (its settings, rules, memory, monitors, profiles, scripts, docs). When deciding where a new file belongs, ask which of those two it is.
- **Promoted bucket** — `engineering/` or `productivity/` under `skills/`. Only promoted skills ship: `sync-manifest.js` scans exactly these buckets into `plugin.json`, the AUTOGEN tables, and the CLI install. `misc/`, `in-progress/`, and `deprecated/` are non-promoted: present in the repo, invisible to the plugin.
- **Declared vs discovered** — `skills` are **declared** in `plugin.json` as directory paths, because bucketed skills sit two levels deep where convention cannot find them. `commands` and `agents` are **discovered** from the repo root and must stay flat; declaring them as file paths is silently ignored by Claude Code, and discovery does not recurse.
...
- **Skill-activator** — the hook (`hooks/skill-activator.js` + `hooks/skill-rules.json`) that scores prompts by keywords/patterns/directories/intents and routes to a skill here or in superpowers.
